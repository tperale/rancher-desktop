...
export default Vue.extend({
  ...
  computed: {
    isFieldDisabled() {
      return !(this.preferences.experimental.virtualMachine.proxy.enabled || this.firstRun);
    },
  },
  methods: {
    onChange<P extends keyof RecursiveTypes<Settings>>(property: P, value: RecursiveTypes<Settings>[P]) {
      if (this.emitChanges) {
        const change = _.set({ experimental: { virtualMachine: { proxy: {} } } }, property, value);

        this.$emit('change', Object.assign(this.preferences.experimental.virtualMachine.proxy, change.experimental.virtualMachine.proxy));
      } else {
